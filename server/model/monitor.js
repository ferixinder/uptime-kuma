...
class Monitor extends BeanModel {
        ...
        const beat = async () => {
            ...
            try {
                if (await Monitor.isUnderMaintenance(this.id)) {
                    ...
                    bean.status = UP;
                    bean.ping = dayjs().valueOf() - startTime;
                } else if (this.type === "plc-client") {
                    let startTime = dayjs().valueOf();

                    console.log("plc");
                    try {
                        await this.connectToPLC(this.hostname);
                        console.log("afterplc");
                        bean.status = UP;
                    } catch (err) {
                        bean.status = DOWN;
                        bean.msg = err;
     ...
     * @returns {Promise<any>}
     */
    connectToPLC(hostname) {
        return new Promise((resolve, reject) => {
            s7client.ConnectTo(hostname, 0, 1, function (err) {
                console.log("plcerr: ", err);
                if (err) {
                    reject(err);
                } else {
                    resolve();
